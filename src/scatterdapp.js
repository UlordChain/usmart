...
const throwIfNoIdentity = () => {
    if(!publicKey) throws('There is no identity with an account set on your Usmart instance.');
};
...
export default class Scatterdapp {
     ...
     * {accounts: [{protocol: "http", blockchain: "uos", host: "114.67.37.2", port: 20580, chainId: "19983e6afbf2ddc5a96337c3be2cc02e6e937fbc3b53f5d55559e10414cc0a07"}]}
     */
    getIdentity(fields = {}){

        // console.log("scatterdapp.getIdentity",fields)
        //GET_OR_REQUEST_IDENTITY 告知前端  IDENTITY内容 
        return _send(NetworkMessageTypes.GET_OR_REQUEST_IDENTITY, {
            network:network,
            fields
        }).then(async identity => {
            
            // console.log(" this.useIdentity 1 ",JSON.stringify(fields))
            this.useIdentity(identity);
            // console.log(" this.useIdentity 2 ",JSON.stringify(fields))
            return identity;
        });
    }
