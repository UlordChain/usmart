...
export default class UOS extends Plugin {

    constructor(){ super(Blockchains.UOS, PluginTypes.BLOCKCHAIN_SUPPORT) }
    ...
    async getEndorsedNetwork(){ //主网络地址                 // 'testnet.uosio.org',
        return new Promise((resolve, reject) => {
            resolve(new Network(
                ...
                8250,
                Blockchains.UOS,
                'cb1025fad7e421fa75e769f4cdd76a2fc4db639ea7732fd299c6d28d14767771'
            ));
        });
    }
    ...
    importAccount(keypair, network, context, accountSelected){
        ...
        const getAccountsFromPublicKey = (publicKey, network) => {
            return new Promise((resolve, reject) => {
                ...
                // console.log({httpEndpoint:`${network.protocol}://${network.hostport()}`})
                eos.getKeyAccounts(publicKey).then(res => {
                    // console.dir(res)
                    if(!res || !res.hasOwnProperty('account_names')){ resolve([]); return false; }
                    // console.log("keypair 之前1", JSON.stringify(keypair))
        ...
        getAccountsFromPublicKey(keypair.publicKey, network).then(accounts => {
            ...
            // console.log("keypair 之前2", JSON.stringify(keypair))
    ...
    privateToPublic(privateKey){ return this.e2u(ecc.privateToPublic(privateKey)); }
    validPrivateKey(privateKey){ return ecc.isValidPrivate(privateKey); }
    validPublicKey(publicKey){   return ecc.isValidPublic(this.u2e(publicKey)); }
    randomPrivateKey(){ return ecc.randomKey(); }

    u2e(pub_u_key){
        const keystr=(pub_u_key);
        if(keystr.indexOf('UOS')==0){
            return "EOS"+keystr.substring(3);
        }
        return pub_u_key;
    }

    e2u(pub_e_key){
        // console.log(typeof(pub_e_key));
       //  console.log(pub_e_key);
        const keystr= (pub_e_key);
        if(keystr.indexOf('EOS')==0){
            return "UOS"+keystr.substring(3);
        }
        return pub_e_key;
    }

    convertsTo(){ //Blockchains.EOS
        return [];
    }
    from_eth(privateKey){
    ...
    async getBalances(account, network, code = 'uosio.token', table = 'accounts'){
        // console.log("1")
    ...
    signatureProvider(...args){
        ...
        // 返回给第三方api 
        // const scatterEOS = scatter.uos(net, Eos, options, net.protocol);
        return (network, _eos, _options = {}, protocol = 'http') => { //此方法是 scatter.uos 的具体参数 _eos 是第三方网站的
