...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async bindBalances(identity){//获取账号信息
            // console.log("step",1)
            ...
            async accountBalances({account, netString}, identity){
                ...
                // netString 正确格式
                // -- // uos:chain:24c2ffcaeef8f222cf658e7d66dd6df9b25da829c006bff00e4c33c3d02df1c5
                //   console.log("step",4)
                //   console.log("step",`${network.host}|${netString}|`)
                //   console.log("cn",`${network.chainId}`)
                if(!network.host){
                    // console.log("nets",JSON.stringify(this.scatter.settings.networks))
                    const hostedNetwork = this.scatter.settings.networks.find(n => n.chainId === network.chainId && n.host !== '');

                    // this.scatter.settings.networks.forEach(n=>{
                    //     n.chainId === network.chainId
                    //    console.log("map",`n：${n.chainId} network：${network.chainId} n,host:${n.host}`)
                    // })

                    // console.log("nets2",hostedNetwork)
        ...
        watch:{
            showingTokens(){
                this.balances = [];
                // console.log("whch",this.showingTokens)
