<template>
    <section class="backup scroller">
        ...
        <section class="panel" v-else>
            ...
            <btn v-on:clicked="destroyclick" :text="locale(langKeys.BUTTON_DestroyScatter)" is-red="true" :margined="true"></btn>
        </section>
    ...
    export default {
        ...
        methods: {
            ...
            destroyclick(){
                    ...
                    // console.log("注销",JSON.stringify(res)) //注销 {"accepted":true}
                    if(!res || !res.hasOwnProperty('accepted')){ 
                        // console.log("注销 accepted","accepted")
                        return false;
                    }
                    this[Actions.DESTROY]().then(() => {
                        this.scatter.settings.hasEncryptionKey=false; //重新注册界面
            ...
            ...mapActions([
                ...
                Actions.SET_SEED,
                Actions.IS_UNLOCKED,
                Actions.DESTROY //方法名字 冲突 导致  destroy() 不执行 
            ])
        }
    }
</script>
