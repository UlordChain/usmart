    ...
    export default {
        ...
        mounted(){
            ...
            if(existing) this.identity = existing.clone();
                ...
                // console.log("init",JSON.stringify(this.identity))
                this.identity.initialize(this.scatter.hash).then(() => {
                // console.log("init_after",JSON.stringify(this.identity))
        ...
        methods: {
            ...
            selectKeypair(keypair){
                //选择 不同密钥后
                // console.log("!keypair.publicKey.length",!keypair.publicKey.length)
                // console.log("publicKey", JSON.stringify(keypair))
                this.selectedKeypair = !keypair.publicKey.length ? null : keypair;

                //-----新增加的12.25--- 好像没有什么用 只是一个标记而已
                if(keypair.publicKey.indexOf("UOS")>-1){
                    this.identity.publicKey=Identity.e2u(this.identity.publicKey)
                }
                else if(keypair.publicKey.indexOf("EOS")>-1){
                    this.identity.publicKey=Identity.u2e(this.identity.publicKey)
                }
            ...
            importAccount(){
                // console.log(this.selectedKeypair)
                // console.log(this.selectedKeypair.publicKey)
                if(!this.selectedKeypair || !this.selectedKeypair.publicKey.length) return false;
                // console.log("next")
                this.importing = true;
                // console.log("导入前", JSON.stringify(this.identity)) 构建账户信息
                AccountService.importFromKey(this.selectedKeypair, this.selectedNetwork, this).then(imported => {
                    ...
                    this.importing = false;
                    //  console.log("导入后", JSON.stringify(this.identity))
            ...
            async saveIdentity(){
                ...
                // console.log("save identity",JSON.stringify(this.identity))
                // return;
                const scatter = this.scatter.clone();
